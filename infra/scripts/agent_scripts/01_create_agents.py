...
p.add_argument("--is_workshop", required=False, default="false")
p.add_argument("--azure_ai_search_connection_name", required=False, default="")
p.add_argument("--azure_ai_search_index", required=False, default="")
args = p.parse_args()
...
is_workshop = args.is_workshop.lower() == "true"
...
# Agent instructions for workshop deployment (Azure SQL + Search tools)
agent_instructions_workshop = '''You are a helpful assistant.

Tool Priority:
    - Always use the **SQL tool** first for quantified, numerical, or metric-based queries.
        - **Always** use the **run_sql_query** function to execute queries.
    ...
    - Always use the **Azure AI Search tool** for summaries, explanations, or insights from documents.
        - **Always** use the search tool when asked about document content, customer issues, or transcripts.
...
# Select instructions based on deployment mode
agent_instructions = agent_instructions_workshop if is_workshop else agent_instructions_fabric
...
# Build tools list based on deployment mode
tools_list = [
