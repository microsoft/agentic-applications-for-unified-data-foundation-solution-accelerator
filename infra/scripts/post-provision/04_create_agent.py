...
def print_config():
    ...
    print(f"\n{'='*60}")
    sql_label = "Fabric Data Agent (Preview)"
...
def build_sql_tool(tables, data_agent_id, data_agent_name,
    ...
    """Build the SQL tool — Fabric Data Agent via MicrosoftFabricPreviewTool"""
    if data_agent_id:
        # Default: use MicrosoftFabricPreviewTool with the CustomKeys connection
...
def create_connections(credential):
    ...
    # Fabric Data Agent preview CustomKeys connection (default SQL tool)
...
def create_agents(project_client, instructions, title_instructions, agent_tools):
    ...
    with project_client:
        ...
        # Create agent
        if DATA_AGENT_ID:
            sql_mode_label = "Fabric Data Agent (Preview)"
        elif DATA_AGENT_MCP_ENDPOINT:
            sql_mode_label = "Fabric Data Agent (MCP fallback)"
        else:
