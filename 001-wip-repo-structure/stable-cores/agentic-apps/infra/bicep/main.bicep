...
param tags object = {}
// ============================================================================
// Parameters - Contact Center Platform
// ============================================================================

@description('Required. Globally unique Azure Communication Services name.')
param communicationServiceName string

@description('Optional. Data location for Azure Communication Services.')
param communicationServiceDataLocation string = 'United States'

@description('Required. Name of the Event Grid system topic for incoming calls.')
param eventGridName string

@description('Optional. HTTPS endpoint for incoming-call notifications.')
@secure()
param incomingCallWebhookEndpoint string?

@description('Optional. Deploy CI workload identity and cost guardrails.')
param shouldEnableCiOidc bool = false

@description('Optional. Name of the CI user-assigned managed identity.')
param ciIdentityName string = 'agentic-apps-ci'

@description('Optional. Immutable numeric GitHub repository owner ID.')
param githubRepositoryOwnerId string?

@description('Optional. Immutable numeric GitHub repository ID.')
param githubRepositoryId string?

@description('Optional. GitHub Environment used in the OIDC subject.')
param githubEnvironment string = 'production'

@description('Optional. Subscription ID containing the Terraform state storage account.')
param stateSubscriptionId string?

@description('Optional. Resource group containing the Terraform state storage account.')
param stateResourceGroupName string?

@description('Optional. Name of the Terraform state storage account.')
param stateStorageAccountName string?

@description('Optional. Name of the resource-group budget.')
param budgetName string = 'agentic-apps-ci-budget'

@description('Optional. Monthly budget amount in billing currency.')
param monthlyBudgetAmount int = 50

@description('Optional. Email address that receives budget notifications.')
param budgetContactEmail string?

@description('Optional. Budget start date at the first day of a month in RFC 3339 format.')
...
@description('Optional. Deploy the WAF-aligned private networking and security landing zone.')
param enablePrivateNetworking bool = false

@description('Optional. Address space for the private networking landing zone.')
...
param appTitlePrimary string = 'Agentic Apps'
...
// ============================================================================
// Module: Contact Center Platform
// ============================================================================
...
module incomingCallEvents './modules/incoming-call-events.bicep' = {
  params: {
    communicationServiceName: communicationServices.outputs.name
    name: eventGridName
    webhookEndpoint: incomingCallWebhookEndpoint
    tags: resourceTags
  }
}

module ciCredentials './modules/ci-credentials.bicep' = if (shouldEnableCiOidc) {
  params: {
    aiServicesName: aiFoundryName
...
module costGuardrail './modules/cost-guardrail.bicep' = if (shouldEnableCiOidc) {
  params: {
...
module landingZone './modules/networking/landing-zone.bicep' = if (enablePrivateNetworking) {
  name: take('module.landing-zone.${solutionName}', 64)
  params: {
    solutionName: solutionSuffix
    location: location
...
@description('Resource ID of the optional private networking landing zone.')
output LANDING_ZONE_VNET_ID string = enablePrivateNetworking ? landingZone!.outputs.virtualNetworkResourceId : ''
@description('Resource ID of Azure Communication Services.')
output COMMUNICATION_SERVICE_ID string = communicationServices.outputs.id

@description('Name of Azure Communication Services.')
output COMMUNICATION_SERVICE_NAME string = communicationServices.outputs.name

@description('Client ID of the CI managed identity when enabled.')
output CI_IDENTITY_CLIENT_ID string? = shouldEnableCiOidc ? ciCredentials!.outputs.clientId : null
