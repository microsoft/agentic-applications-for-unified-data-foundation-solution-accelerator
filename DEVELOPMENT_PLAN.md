...
| Create Fabric workspace in `02_create_fabric_items.py` | Extend `scripts/02_create_fabric_items.py` to create the Fabric workspace if one does not already exist, using the capacity provisioned by `azd up`. The created workspace ID should be written back to the environment so subsequent steps can reference it without manual configuration. | `scripts/02_create_fabric_items.py` | ☐ |
...
| Make all app text configurable per scenario pack | All user-visible text in the frontend — including the browser tab title in `index.html`, the app header/title, and the chat landing message — should be driven by the active scenario pack. Add text fields to each registry entry in `scenarios.py` and have `08_app_deployment.py` set the corresponding app settings so no text is hard-coded in the frontend build. | `scripts/scenarios.py`, `scripts/08_app_deployment.py`, `src/App/`, `src/App/index.html` | ☐ |
