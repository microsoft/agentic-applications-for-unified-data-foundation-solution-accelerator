...
def mock_sql_dependencies():
    ...
    with patch("history_sql.get_fabric_db_connection") as mock_get_conn, patch("history_sql.pyodbc") as mock_pyodbc:
        ...
        yield {"get_connection": mock_get_conn, "pyodbc": mock_pyodbc, "connection": mock_conn, "cursor": mock_cursor}
...
class TestGetFabricDBConnection:
    ...
    async def test_get_connection_dev_mode_driver18(self, monkeypatch):
        ...
        with patch("history_sql.AzureCliCredential", return_value=mock_credential), patch(
            "history_sql.pyodbc.connect"
        ) as mock_connect:
...
class TestRunNonQueryParams:
    ...
    async def test_run_nonquery_success(self, mock_db_connection):
        ...
        with patch("history_sql.get_fabric_db_connection", return_value=mock_db_connection):
            result = await run_nonquery_params("DELETE FROM conversations WHERE id = ?", ("conv_123",))
    ...
    async def test_run_nonquery_with_params(self, mock_db_connection):
        ...
        with patch("history_sql.get_fabric_db_connection", return_value=mock_db_connection):
            result = await run_nonquery_params(
                "UPDATE conversations SET title = ? WHERE id = ? AND userId = ?", ("New Title", "conv_123", "user_123")
            )
...
class TestRunQueryParams:
    ...
    async def test_run_query_with_params(self, mock_db_connection):
        ...
        mock_db_connection.cursor().fetchall.return_value = [("conv_123", "user_123", "My Conversation")]

        with patch("history_sql.get_fabric_db_connection", return_value=mock_db_connection):
            result = await run_query_params("SELECT * FROM conversations WHERE userId = ?", ("user_123",))
...
class TestSQLEndpoints:
    ...
    async def test_create_conversation_endpoint(self, client, mock_sql_dependencies):
        ...
        with patch("history_sql.get_fabric_db_connection", return_value=mock_sql_dependencies["connection"]):
            response = client.post("/history/conversation", json={"userId": "user_123", "title": "Test Conversation"})
...
class TestDeleteConversationFunction:
    ...
    async def test_delete_conversation_calls_nonquery(self):
        ...
        with patch("history_sql.run_query_params", new_callable=AsyncMock) as mock_query, patch(
            "history_sql.run_nonquery_params", new_callable=AsyncMock
        ) as mock_run:
...
class TestUpdateConversationFunction:
    ...
    async def test_update_conversation_new_messages(self, mock_db_connection):
        ...
        with patch("history_sql.run_query_params", new_callable=AsyncMock) as mock_query, patch(
            "history_sql.create_message", new_callable=AsyncMock
        ) as mock_create:
    ...
    async def test_update_conversation_with_title(self, mock_db_connection):
        ...
        with patch("history_sql.run_query_params", new_callable=AsyncMock) as mock_query, patch(
            "history_sql.generate_title", new_callable=AsyncMock
        ) as mock_title, patch("history_sql.create_conversation", new_callable=AsyncMock), patch(
            "history_sql.create_message", new_callable=AsyncMock
        ) as mock_create:
            mock_query.side_effect = [
...
class TestEndpointIntegration:
    ...
    async def test_list_conversations_endpoint_success(self):
        ...
        with patch("history_sql.get_authenticated_user_details") as mock_auth, patch(
            "history_sql.get_conversations", new_callable=AsyncMock
        ) as mock_get, patch("history_sql.track_event_if_configured"):
    ...
    async def test_read_conversation_endpoint_success(self):
        ...
        with patch("history_sql.get_authenticated_user_details") as mock_auth, patch(
            "history_sql.get_conversation_messages", new_callable=AsyncMock
        ) as mock_get, patch("history_sql.track_event_if_configured"):
            mock_auth.return_value = {"user_principal_id": "user123"}
    ...
    async def test_read_conversation_endpoint_no_id(self):
        ...
        with patch("history_sql.get_authenticated_user_details") as mock_auth, patch(
            "history_sql.track_event_if_configured"
        ):
            mock_auth.return_value = {"user_principal_id": "user123"}
    ...
    async def test_delete_conversation_endpoint_success(self):
        ...
        with patch("history_sql.get_authenticated_user_details") as mock_auth, patch(
            "history_sql.delete_conversation", new_callable=AsyncMock
        ) as mock_delete, patch("history_sql.track_event_if_configured"):
            mock_auth.return_value = {"user_principal_id": "user123"}
    ...
    async def test_delete_all_conversations_endpoint_success(self):
        ...
        with patch("history_sql.get_authenticated_user_details") as mock_auth, patch(
            "history_sql.get_conversations", new_callable=AsyncMock
        ) as mock_get, patch("history_sql.delete_all_conversations", new_callable=AsyncMock) as mock_delete, patch(
            "history_sql.track_event_if_configured"
        ):
            mock_auth.return_value = {"user_principal_id": "user123"}
    ...
    async def test_rename_conversation_endpoint_success(self):
        ...
        with patch("history_sql.get_authenticated_user_details") as mock_auth, patch(
            "history_sql.rename_conversation", new_callable=AsyncMock
        ) as mock_rename, patch("history_sql.track_event_if_configured"):
            mock_auth.return_value = {"user_principal_id": "user123"}
    ...
    async def test_update_conversation_endpoint_success(self):
        ...
        with patch("history_sql.get_authenticated_user_details") as mock_auth, patch(
            "history_sql.update_conversation", new_callable=AsyncMock
        ) as mock_update, patch("history_sql.track_event_if_configured"):
            mock_auth.return_value = {"user_principal_id": "user123"}
            mock_update.return_value = {"id": "conv123", "title": "Test", "updatedAt": "2024-01-01"}
...
class TestDatabaseConnectionPaths:
    ...
    async def test_get_fabric_db_connection_prod_mode_driver17_fallback(self):
        ...
        with patch("history_sql.os.getenv") as mock_env, patch("history_sql.pyodbc.connect") as mock_connect, patch(
            "history_sql.AzureCliCredential"
        ):
            ...
            mock_connect.side_effect = [Exception("Driver 18 failed"), MagicMock()]  # Driver 17 succeeds
    ...
    async def test_get_conversation_messages_no_user_id(self):
        ...
        with patch("history_sql.run_query_params", new_callable=AsyncMock) as mock_query:
            mock_query.return_value = [{"role": "user", "content": "msg1", "citations": "", "feedback": ""}]
...
class TestMessageContentProcessing:
    ...
    async def test_generate_title_service_response_exception(self):
        ...
        with patch("history_sql.AZURE_AI_AGENT_ENDPOINT", "http://test"), patch(
            "history_sql.AIProjectClient"
        ) as mock_client, patch("history_sql.AzureAIClient"), patch("history_sql.ChatAgent"):
...
class TestUpdateConversationEdgeCases:
    ...
    async def test_update_conversation_only_assistant_message(self):
        ...
        request_json = {
            "conversation_id": "conv123",
            "messages": [{"role": "assistant", "content": "Response", "id": "msg1"}],
        }
    ...
    async def test_update_conversation_creates_new_conversation(self):
        ...
        with patch("history_sql.run_query_params", new_callable=AsyncMock) as mock_query, patch(
            "history_sql.generate_title", new_callable=AsyncMock
        ) as mock_title, patch("history_sql.create_conversation", new_callable=AsyncMock) as mock_conv, patch(
            "history_sql.create_message", new_callable=AsyncMock
        ) as mock_create:
            mock_query.side_effect = [
...
class TestEndpointValidation:
    ...
    async def test_delete_all_conversations_endpoint_delete_fails(self):
        ...
        with patch("history_sql.get_authenticated_user_details") as mock_auth, patch(
            "history_sql.get_conversations", new_callable=AsyncMock
        ) as mock_get, patch("history_sql.delete_all_conversations", new_callable=AsyncMock) as mock_delete:
            mock_auth.return_value = {"user_principal_id": "user123"}
...
class TestGenerateTitleEdgeCases:
    ...
    async def test_generate_title_returns_none_from_agent(self):
        ...
        with patch("history_sql.AZURE_AI_AGENT_ENDPOINT", "http://test"), patch(
            "history_sql.AIProjectClient"
        ) as mock_client, patch("history_sql.AzureAIClient") as mock_ai_client, patch(
            "history_sql.ChatAgent"
        ) as mock_agent:
            # Setup mocks
...
class TestAzureSQLConnection:
    """Tests for Azure SQL Server connection functionality."""

    @pytest.mark.asyncio
    async def test_get_azure_sql_connection_driver18_success(self, monkeypatch):
        """Test successful Azure SQL connection with driver 18."""
        from history_sql import get_azure_sql_connection

        monkeypatch.setenv("SQLDB_SERVER", "test-server.database.windows.net")
        monkeypatch.setenv("SQLDB_DATABASE", "testdb")
        monkeypatch.setenv("API_UID", "test-uid")

        with patch("history_sql.get_azure_credential_async") as mock_cred, patch(
            "history_sql.pyodbc.connect"
        ) as mock_connect:
            ...
            result = await get_azure_sql_connection()
    ...
    @pytest.mark.asyncio
    async def test_get_azure_sql_connection_driver17_fallback(self, monkeypatch):
        """Test Azure SQL connection falls back to driver 17."""
        from history_sql import get_azure_sql_connection
        ...
        with patch("history_sql.get_azure_credential_async") as mock_cred, patch(
            ...
            # First call with driver 18 fails, second with driver 17 succeeds
            mock_connection = Mock()
            mock_connect.side_effect = [Exception("Driver 18 not found"), mock_connection]
    ...
    @pytest.mark.asyncio
    async def test_get_azure_sql_connection_both_drivers_fail(self, monkeypatch):
        """Test Azure SQL connection when both drivers fail."""
        from history_sql import get_azure_sql_connection
        ...
        with patch("history_sql.get_azure_credential_async") as mock_cred, patch(
            ...
            # Both drivers fail
            mock_connect.side_effect = [Exception("Driver 18 not found"), Exception("Driver 17 not found")]
...
class TestApplicationInsightsMissing:
    """Tests for missing Application Insights code paths."""

    def test_module_import_without_app_insights(self, monkeypatch):
        """Test module import when Application Insights is not configured."""
        import importlib
        import sys

        monkeypatch.delenv("APPLICATIONINSIGHTS_CONNECTION_STRING", raising=False)

        # Reload module to test import-time code
        if "history_sql" in sys.modules:
            del sys.modules["history_sql"]

        # Should import successfully even without instrumentation key
        import history_sql

        assert history_sql is not None


class TestFabricDriverFallback:
    """Tests for Fabric DB connection driver fallback."""

    @pytest.mark.asyncio
    async def test_get_fabric_db_driver17_fallback_on_prod(self, monkeypatch):
        """Test Fabric connection falls back to driver 17 in production."""
        from history_sql import get_fabric_db_connection

        monkeypatch.setenv("AZURE_ENV_ONLY", "false")
        monkeypatch.setenv("FABRIC_LAKEHOUSE_SQL_ENDPOINT", "test-fabric.database.windows.net")
        monkeypatch.setenv("FABRIC_LAKEHOUSE_DATABASE", "testdb")
        monkeypatch.setenv("API_UID", "test-uid")
        ...
        with patch("history_sql.get_azure_credential_async") as mock_cred, patch(
            ...
            # Driver 18 fails, driver 17 succeeds
            mock_connection = Mock()
            mock_connect.side_effect = [Exception("Driver 18 error"), mock_connection]
...
class TestSqlQueryToolEdgeCases:
    """Tests for SqlQueryTool edge cases."""

    @pytest.mark.asyncio
    async def test_sqlquerytool_with_decimal_and_datetime(self):
        """Test SqlQueryTool handling of Decimal and datetime types."""
        from history_sql import SqlQueryTool
        from decimal import Decimal
        import pyodbc

        with patch("history_sql.pyodbc") as mock_pyodbc:
            mock_conn = Mock(spec=pyodbc.Connection)
            mock_cursor = Mock()
            mock_cursor.description = [("price",), ("created_date",), ("updated_at",)]

            # Mock row with Decimal and datetime values
            mock_row = (Decimal("99.99"), date(2024, 1, 15), datetime(2024, 1, 15, 10, 30, 0))
            ...
            tool = SqlQueryTool(pyodbc_conn=mock_conn)
            result = await tool.run_sql_query("SELECT price, created_date, updated_at FROM test")

            assert result is not None
            assert len(result) == 1
            assert isinstance(result[0]["price"], float)
            assert isinstance(result[0]["created_date"], str)
            assert isinstance(result[0]["updated_at"], str)

    @pytest.mark.asyncio
    async def test_sqlquerytool_query_error(self):
        """Test SqlQueryTool error handling during query execution."""
        from history_sql import SqlQueryTool
        import pyodbc
        ...
        tool = SqlQueryTool(pyodbc_conn=mock_conn)
        result = await tool.run_sql_query("INVALID SQL")
