// ============================================================================
// STANDALONE TEST TEMPLATE — Container Registry + Role Assignments (bicep variant)
// ----------------------------------------------------------------------------
...
//
// It reuses the exact same modules that infra_new/bicep/main.bicep uses:
...
//   az deployment group create -g rg-cr-test -f infra_new/test/test-container-registry-bicep.bicep
...
// ----------------------------------------------------------------------------
module hostingPlan '../bicep/modules/compute/app-service-plan.bicep' = if (deployAppService) {
...
module appService '../bicep/modules/compute/app-service.bicep' = if (deployAppService) {
...
// ----------------------------------------------------------------------------
// Container App (optional) — needs a Log Analytics workspace for the environment
// ----------------------------------------------------------------------------
resource logAnalytics 'Microsoft.OperationalInsights/workspaces@2023-09-01' = if (deployContainerApp) {
...
module containerAppEnvironment '../bicep/modules/compute/container-app-environment.bicep' = if (deployContainerApp) {
...
module containerApp '../bicep/modules/compute/container-app.bicep' = if (deployContainerApp) {
...
// ----------------------------------------------------------------------------
module containerRegistry '../bicep/modules/compute/container-registry.bicep' = if (!useExistingContainerRegistry) {
...
// ----------------------------------------------------------------------------
module roleAssignments '../bicep/modules/identity/role-assignments.bicep' = {
